...
"""
This is the implementation code, the implementation aims to batch mutations to then process multiple mutations, hopefully faster run time.
"""

#prioritize test cases based on coverage or runtime
#def prioritize_testcases(testcases):
#
#	testcases.sort(key=lambda tc: len(tc['new_hex_files']), reverse=True)
#	return testcases[:len(testcases) // 2]

#batch_mutate helper function

def batch_mutate(testcases, mutation_prob, *args):

	mutation_results = []
	for tc in testcases:
		for hex_file_out in tc['new_hex_files']:
			result = prog_mut.mutate_prog(tc['hex_file'], hex_file_out, mutation_prob, *args)
			mutation_results.append(result)
	return mutation_results

#batch muatate parallel function

#def batch_mutate_parallel(testcases, mutation_prob, *args):
#	def mutate_task(tc):
#		for hex_file_out in tc['new_hex_files']:
#			return prog_mut.mutate_prog(tc['hex_file'], hex_file_out, mutation_prob,*args)
#	
#	with ThreadPoolExecutor() as executor:
#		results = executor.map(mutate_task, testcases)
#	return list(results)

#Modified run_muts function to include the batch mutate helper function

def run_muts(testcases_to_mut, optimizer_sol, nop_inst_bin_32, inst_list_all_w_ext):
	mutation_prob = 100

	#Priority testcases
#	testcases_to_mut = prioritize_testcases(testcases_to_mut)

	#Use batch function for mutation
	mutation_results = batch_mutate(
		testcases_to_mut,
		mutation_prob,
		optimizer_sol, 
		nop_inst_bin_32,
	...
	#Collect the mutated testcases for simulation
	num_testcases_generated = sum(len(tc['new_hex_files']) for tc in testcases_to_mut)

	#Return total number of generated test cases
