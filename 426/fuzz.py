...
def calc_no_times_to_mut(num_times_to_mut, times_mutated\
    ...
    if cov_incr > 0:
        ...
        # decreasing factor i.e., decr_factor. Make sure that
        # num_times_to_mut_local remains a integer

        # Ensure mutation count is an integer.
        ...
        # Enforce a minimum mutation count.
        ...
        if times_mutated == 0: # this is a generated input, dint mutate yet
            ...
            num_times_to_mut_local = random.randint(0, max(0, num_times_to_mut // 2 - 1))
            # EDIT 3 END
...
def compute_cov_achieved(merged_cov_dict, tot_cov_points):
    ...
    merged_cov_points_dict = {}
    for coverage_type, coverage_string in merged_cov_dict.items():
        # Count the number of '1's in the coverage string for this type.
        merged_cov_points_dict[coverage_type] = coverage_string.count('1')

    # Sum up the coverage points from all types to get the total coverage points achieved.
    cov_points_ach = sum(merged_cov_points_dict.values())
    ...
    if tot_cov_points > 0:  # Prevent division by zero.
        cov_per_ach = (cov_points_ach / tot_cov_points) * 100  # Calculate percentage.
    else:
        cov_per_ach = 0  # No coverage points to divide by.
    # Round off the percentage to two decimal places for better readability (optional).
    cov_per_ach = round(cov_per_ach, 2)
