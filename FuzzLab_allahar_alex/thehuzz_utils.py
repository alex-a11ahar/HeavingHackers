...
class DATABASE: 
    ...
    def get_testcases_to_sim(self, no_testcases):
        ...
        if no_testcases > self.num_new_testcases():  # Check if the no_testcases is greater than the number of testcases available from the DATABASE class.
            print(f"Error: Requested {no_testcases} testcases, but only {self.num_new_testcases()} are available.") # Error message for error handling
            exit()  # Stop the fuzzer if there aren't enough testcases.
        ...
        # Using the psuedo code above the we get the first no_testcases elements from `elf.new_testcases array and assign them to self.testcases_to_sim.
        self.testcases_to_sim = self.new_testcases[0:no_testcases]
