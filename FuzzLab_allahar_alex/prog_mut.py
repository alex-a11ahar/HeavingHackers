...
def bitflip(mut, inst_type, num_bits):
    ...
    # mut is the binary value of the instruction to mutate
    # first convert it into an array of bits
    mut_list = [b for b in mut]

    # for each bit index to mutate, flip the bit from the mut_list
    for i in m_index:
        mut_list[i] = '0' if mut_list[i] == '1' else '1'

    # join the bits to get the mutated instruction
    mut = ''.join(mut_list)
...
def my_random(mut, inst_type, num_bits, bits_type):
    ...
    mut_list = list(mut) # Convert the mut binary string value to a list for to loop through.

    # For loop to go through indices in m_index.
    for i in m_index:
        # 1 becomes 0 and 0 becomes 1 they flip
        if mut_list[i] == '1': #If 1
            mut_list[i] = '0' # Go 0
        else:
            mut_list[i] = '1' #If 0, go 1
           
    mut = ''.join(mut_list) # Combine the mut list that is randomly changed back to a binary string.
...
def mutate_prog(hex_file_in, hex_file_out, mutation_prob\
    ...
    val_mut = [0,1,2,3,4,5,6,7] #Valid mutations-> mutations that are used by
    opcode_mut_list = [8,9,10,11]  #these mutation techniques can change the instruction type
    mut_state = 0
    line_number = 0
    inst_bin_i_prev = -1
    inst_list_fr_clone = []
